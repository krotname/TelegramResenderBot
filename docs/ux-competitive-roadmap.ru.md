...
Статус после релиза `1.1.0`: раздел 7 использован как acceptance baseline для
bugfix/UX hardening релиза. Дальнейшая итерационная разработка начинается с
раздела 8 (`v1.2.0` и далее).
